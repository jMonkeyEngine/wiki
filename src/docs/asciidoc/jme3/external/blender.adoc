...
Once this is done, export your model with one of the supported <<jme3/features#supported-formats,Supported External File Types>> and convert it into a `.j3o` with the SDK, or the JME link:{link-javadoc}/com/jme3/export/binary/BinaryExporter.html[BinaryExporter].
