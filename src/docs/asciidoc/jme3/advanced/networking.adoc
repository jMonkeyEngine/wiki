...
You register message types to the com.jme3.network.serializing.Serializer only on the server! SpiderMonkey has an automatic registering mechanism that will register the messages on the client the first time it connects to the server.

[WARNING]
====
Messages must be registered after server creation, and before it is started. NOT before the server is created.
====
