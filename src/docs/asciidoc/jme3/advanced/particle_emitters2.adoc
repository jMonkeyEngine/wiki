...
a| `setNumParticles()`
<a|
3+a| The maximum number of particles visible at the same time. Specified by user in constructor.
...
a| 20
3+a| Density of the effect, how many new particles are emitted per second. +
...
a| 0.2f, 2f
3+a| The radius of the scaled sprite image. Set both to same value for constant size effect. +
...
a| gray
3+a| Controls how the opaque (non-black) parts of the texture are colorized. +
...
a| Vector3f(0,0,0)
3+a| A vector specifying the initial direction and speed of particles. The longer the vector, the faster.
...
a| 0.2f
3+a| How much the direction (`setInitialVelocity()`) can vary among particles. Use a value between 1 and 0 to create a directed swarm-like cloud of particles. +
...
a| `setFacingVelocity()`
a| false
3+a| true = Flying particles pitch in the direction they're flying (e.g. missiles). +
...
a| `setFaceNormal()`
a| Vector3f.NAN
3+a| Vector3f = Flying particles face in the given direction (e.g. horizontal shockwave faces up = Vector3f.UNIT_Y). +
...
a| `setRotateSpeed()`
a| 0f
3+a| 0 = Flying particles don't spin while flying (e.g. smoke, insects, controlled projectiles). +
...
a| `setRandomAngle()`
a| false
3+a| true = The particle sprite is rotated at a random angle when it is emitted (e.g. explosion, debris). +
...
3+a| Particles fall in the direction of the vector (e.g. debris, sparks). +
...
a|EmitterPointShape()
3+a| By default, particles are emitted from the emitters location (a point). You can increase the emitter shape to occupy a sphere, so that the start point of new particles can be anywhere inside the sphere, which makes the effect a bit more irregular.
