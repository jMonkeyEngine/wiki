...
[TIP]
====
You can clear the Blender buffers by using the `Browse` button located next to the material, texture or UV Image to load the item to be cleared, then while holding the kbd:[Shift] button down, click the btn:[X] button located next to the loaded item. The exception to this rule is the `Linked Action` buffer for animations. To clear this buffer see <<jme3/advanced/mixamo#clearing-the-linked-action-buffer#,Clearing The Linked Action Buffer>> in this tutorial.
====
