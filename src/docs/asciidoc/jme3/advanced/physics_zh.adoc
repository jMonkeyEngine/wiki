...
*  這個碰撞形狀在空間表面有變化時不會自動更新
**  你可以重新連接控制或自行使用CollisionShapeFactory來更新它
...
*  *當物理物件的空間形狀超出螢幕圖形時它會留在物理空間裡*
**  使用物理空間.移除(物理物件)或只是物理控制.setEnabled(false);用來將他們從物理空間移除
...
**  你可以使用 PhysicsTickListener介面然後增加到物理空間中
...
a|Positions the objects. Do not use setLocalTranslation() for physical objects. 重點: Make certain not to make CollisionShapes overlap when positioning them. 
