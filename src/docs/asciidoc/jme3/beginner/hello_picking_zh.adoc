= jMonkeyEngine 3 教學(8) - 哈囉 選取(Picking)
:author:
:revnumber:
...
前一個: <<jme3/beginner/hello_animation#,哈囉 動畫(Animation)>>,
下一個: <<jme3/beginner/hello_collision#,哈囉 碰撞 (Collision)>>
...
This tutorial relies on what you have learned in the <<jme3/beginner/hello_input_system#,哈囉 輸入>> tutorial. You find more related code samples under <<jme3/advanced/mouse_picking#,滑鼠選取>> and <<jme3/advanced/collision_and_intersection#,Collision and Intersection>>.
