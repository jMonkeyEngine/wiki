= jMonkeyEngine 3 教學(5) - 哈囉 輸入系統(Input System)
:author: 
:revnumber: 
...
前一個: <<jme3/beginner/hello_main_event_loop#,哈羅 更新迴圈(Update Loop)>>,
下一個: <<jme3/beginner/hello_material#, 哈囉 材質(Material)>>
...
**  小撇步: 使用 <<jme3/faq#how-do-i-switch-between-third-person-and-first-person-view,flyCam.setEnabled(false);>> 

.  Modify the mappings so that you can also trigger the up an down motion with the mouse scroll wheel!
**  小撇步: 使用 new MouseAxisTrigger(MouseInput.AXIS_WHEEL, true)
