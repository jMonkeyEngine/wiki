...
Под ресурсами(assets) мы имеем в виду мультимедийные файлы, такие как 3D-модели, материалы, текстуры, сцены, пользовательские шейдеры, музыку и звуковые файлы и пользовательские шрифты. JME3 имеет встроенный менеджер ресурсов, который поможет вам хранить ваши ресурсы проекта, организованного. Think of the asset manager as the filesystem of your game, independent of the actual deployment platform. By default, store your assets in the `MyGame/assets/ ` directory of your project.
...
См. также <<jme3/intermediate/best_practices#,Best Practices>>.
