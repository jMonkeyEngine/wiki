...
Applying impulses or checking for overlaps only has an effect right at a physics update cycle, which is not every frame. If you do physics interactions at arbitrary spots in the simpleUpdate() loop, calls will be dropped at irregular intervals, because they happen out of cycle.


=== When (Not) to Use Tick Listener?

When you write game mechanics that apply impulses, you must implement a tick listener (com.jme3.bullet.PhysicsTickListener) for it. The tick listener makes certain the impulses are applied in time for the next physics tick.
...
*  `prePhysicsTick(PhysicsSpace space, float tpf)` is called before each step, here you apply impulses (change the state).
*  `physicsTick(PhysicsSpace space, float tpf)` is called after each step, here you poll the results (get the current state).

The tpf argument is simulation step in seconds, not the time between frames.
